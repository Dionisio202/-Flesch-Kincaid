...
public class ActiveObject<T, R>
    ...
    protected RunnableFuture<R> makeThreadClosure(Function<T, R> function,
        ...
        mRunnableFuture = new FutureTask<>(() -> function.apply(params));
        ...
        //  with the proper code.
        mThread = Thread.ofVirtual().unstarted(mRunnableFuture);
    ...
    (Function<Map.Entry<K, V>, R> task,
     Map<K, V> inputMap) {
        ...
        // proper code.
        ActiveObjectArray<K, V, R> array = new ActiveObjectArray<>();
        for (Map.Entry<K, V> entry : inputMap.entrySet()) {
            array.add(new ActiveObject<>(task, entry));
        }
        return array;
    }
