...
public class BardPlayAnalyzer
    ...
    protected ActiveObjectArray<String, String, Double> makeActiveObjects() {
        ...
        // TODO -- Replace 'return null' with the correct solution.
        return ActiveObject.makeActiveObjects(
                this::processInput,
                sBardMap
        );
    }
    ...
    protected Array<String> processActiveObjects
        ...
        for (ActiveObject<Map.Entry<String, String>, Double> ao : activeObjects) {
            try {
                // Get the computed score from each ActiveObject
                Double score = ao.get();
                // Format the result and add to results
                results.add(makeStringResult(ao, score));
            } catch (InterruptedException | ExecutionException e) {
                Thread.currentThread().interrupt();
                System.err.println("Error processing ActiveObject: " + e.getMessage());
            }
        }
        return results;
    }
